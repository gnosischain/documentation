...
Instructions for deploying bridge mediator contracts on top of [the Arbitrary Message Bridge \(AMB\) between the Ethereum Mainnet and the Gnosis Chain](/bridges/eth-gc).
...
![](/img/bridges/image-36.png)

5. Verify that tokens can be transferred in both directions through the bridge. See the [the sUSD bridge AMB extension](/bridges/eth-gc/extensions/susd/transfer) for an example process.
