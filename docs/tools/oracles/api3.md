...
- [**`airnode`**](awd): Specifies the Airnode Address.
- [**`endpointId`**](awd): Specifies which endpoint to be used.
- [**`sponsor`**](awd) and [**`sponsorWallet`**](awd): Specifies which wallet will be used to fulfill the request.
- [**`parameters`**](awd): Specifies the API and Reserved Parameters (see [Airnode ABI specifications](awd) for how these are encoded). Parameters can be encoded off-chain using [`@airnode-abi`](awd) library.

### Response parameters

The callback to the Requester contains two parameters:

- [**`requestId`**](awd): First acquired when making the request and passed here as a reference to identify the request for which the response is intended.
- [**`data`**](awd): In case of a successful response, this is the requested data which has been encoded and contains a timestamp in addition to other response data. Decode it using the `decode()` function from the `abi` object.
...
[dAPIs](awd) are continuously updated streams of off-chain data, such as the latest cryptocurrency, stock and commodity prices.

dAPIs are composed of [**Beacons**](awd), which are first-party data feeds. A Beacon is directly powered by the owner of the data, the API provider. Compared to third-party oracle solutions, which involve middlemen node operators, this approach is secure, transparent, cost-efficient and scalable.

![API3 Remix deploy](</img/tools/api3/SS4.png>)


Developers use the [DapiServer.sol➚](awd) contract to access dAPIs. `DapiServer.sol` reads directly from its data store of Beacons, which are powered by API provider-owned and operated [Airnodes](awd).
...
Here's an example of a contract using a dAPI on Gnosis Chain
...
uint32 public timestamp;
    ...
    function callDapi(
        ...
        // Call the DapiServer contract for the value and timestamp of AVAX/USD on Gnosis(testnet).
...
### Calling a dAPI using Remix on Gnosis Chain

To call a dAPI using Remix IDE, [Click Here](awd).
...
As we are going to deploy the Contract on Gnosis Chain, make sure you have enough funds in your wallet to deploy the Contract.
...
Because you are calling a dAPI on a testnet, a subscription for the dAPI AVAX/USD on Gnosis Chain is not needed. However you will need to get permission for your smart contract to call the dAPI.

Copy the address of the contract. Select copy icon to the right of the contract name, `MyContract.sol`.

Go to the API3 Market page for [AVAX/USD on Gnosis Chain➚](awd). Enter the contract address into the permissions widget to the right side of the page.
...
- `_dapiServerContractAddress` - The contract address for the `DapiServer.sol` contract on Gnosis Chain. Other network addresses for `DapiServer.sol` can be found below.

- `_dapi` - A `bytes32` representation of AVAX/USD. Use `utils.formatBytes32String("AVAX/USD");` to get this encoded value. Try it in the [ethers playground➚](awd).
